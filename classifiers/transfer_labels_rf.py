...
def main():
	...
	if saveconfig["training"] == filemode.READ:
		...
		rf_file = open(savefiles["classifier"], 'rb')
		classifier = joblib.load(rf_file)
		rf_file.close()
		...
		classifier = trainForest(features, labels, num_src_superpixels)

		##################
		## SAVE FOREST: ##
		##################

		print("Saving forrest...")
		start_time = time.time()

		rf_file = open(savefiles["classifier"], 'wb')
		joblib.dump(classifier, rf_file, compress=saveconfig["classifier_compression"])
		rf_file.close()

		end_time = time.time()
		elapsed_time = end_time - start_time
		print("Saving forest took %.1f seconds" % elapsed_time)
	...
	if saveconfig["prediction"] == filemode.READ:
		...
		pred = classifier.predict(search_features)
		pred_labels = [id_label_db[p] for p in pred]
...
def trainForest(features, labels, total_features):

	start_time = time.time()

	classifier = RandomForestClassifier(n_estimators=100, max_features=None, n_jobs=CLASSIFIER_JOBS)
